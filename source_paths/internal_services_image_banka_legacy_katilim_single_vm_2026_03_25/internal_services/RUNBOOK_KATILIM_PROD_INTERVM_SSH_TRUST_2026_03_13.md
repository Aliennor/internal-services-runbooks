# Katilim Prod Inter-VM SSH Trust Runbook

Use this to make the two prod VMs trust each other for passwordless SSH.
...
```bash
nc -vz 10.210.28.27 22
```

From `VM2`:

```bash
nc -vz 10.210.28.26 5432
```
...
- replace `10.210.28.26` with the new active node IP
- replace `10.210.28.27` with the new passive node IP
