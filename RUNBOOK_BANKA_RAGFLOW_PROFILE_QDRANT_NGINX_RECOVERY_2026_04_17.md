...
The corrected Banka HA bootstrap source now does three things:

- defaults `ENABLE_QDRANT=false`
- starts RAGFlow with `--profile "${DOC_ENGINE:-elasticsearch}" --profile "${DEVICE:-cpu}"`
- starts RAGFlow before `openweb-ui`/`nginx`

Until a new installer bundle containing that source is published, use this
runbook after extracting the current installer image.
